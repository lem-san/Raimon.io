const employment = {
    en: [
        {
            id: 1,
            title: "ALT English Instructor",
            company: "AtoZ",
            link: "https://atoz-ed.com/",
            desc: "Facilitate ESL English classes as the primary planner and instructor in collaboration with Japanese homeroom teachers. Design and deliver dynamic, student-centered lessons tailored to varying language levels and learning styles. Develop original teaching materials and a flexible curriculum that supports student growth through scaffolded activities, active learning, and real-world language use. Create interactive web apps and digital games to enhance classroom engagement and independent learning opportunities. Build custom digital tools for vocabulary review, letter recognition and spelling, and speaking exercises. Coordinate cross-school collaborative projects that connect students within our local community.",
            dates: "2021 - Present",
            softSkills: ["Curriculum Development", "Student Assessment", "ICT Literacy Development", "Game Creation"],
            technicalSkills: ["HTML & CSS", "Javascript", "React", "Google Cloud Services"] 
        },
        {
            id: 2,
            title: "Remote ESL Instructor",
            company: "Metaverse",
            link: "https://metaverse.skytalk.co.jp/",
            desc: "Conduct remote English learning lessons in the context of the students' favorite games. Play with the students during joint game sessions, using English to communicate to accomplish in-game activities and goals. Develop personalised curriculum and materials to match their learning pace and their interests. Track student learning  progress and their ICT literacy.",
            dates: "2025 - Present",
            softSkills: ["Remote Learning", "Curriculum Development", "ICT Literacy Development"],
            technicalSkills: [] 
        },
        {
            id: 3,
            title: "Event Coordinator",
            company: "Synergy Events Group",
            link: "https://www.synergyeventsgroup.com.au/",
            desc: " Oversaw the setup, pack-down, and ongoing maintenance of event styling and technical components, ensuring smooth execution. Coordinated event logistics including scheduling, venue layouts, and technical requirements, with a strong focus on efficiency and adaptability. Liaised with venue management, clients, and on-site staff to ensure styling and AV solutions aligned with each event's unique vision and constraints. Managed installation of custom venue theming, floral arrangements, and basic lighting/audio systems tailored to client specifications. Independently managed warehouse inventory, including stock intake/outtake and detailed recordkeeping to maintain equipment availability and quality checks. Designed and implemented an optimized warehouse layout to streamline the daily movement of event equipment, improving workflow across multiple concurrent events.",
            dates: "2015 - 2021",
            softSkills: ["Event Management", "Logistics Coordination", "Inventory Management", "Venue Styling", "Team Leadership", "Problem Solving"],
            technicalSkills: []
        },
        {
            id: 4,
            title: "IT Support Technician",
            company: "NTC",
            link: "https://www.ntc.gov.au/",
            desc: "IT Tech Support: Database management & recordkeeping of integrated EDRMS. Troubleshooting user issues, software & hardware failure. System administration & user profile management. Content migration and structuring of new CMS. Trained staff on appropriate usage, workflows, and general site management. Part of the team responsible for migrating 20+ years of legacy data into a new EDRMS and a subsequent new website for public viewing. Additionally, General Administration & Reception responsibilities.",
            dates: "2013 - 2015",
            softSkills: ["System Administration", "Data Migration", "Technical Support", "Staff Training", "Troubleshooting"],
            technicalSkills: ["Database Management", "SQL", "Networking"]
        }
    ],
    ja: [
        {
            id: 1,
            title: "ALT英語講師",
            company: "AtoZ",
            link: "https://atoz-ed.com/",
            desc: "日本人担任の先生と協力して、主に自ら計画・実施するESL英語の授業を担当。学習者のレベルやスタイルに合わせた、生徒主体のダイナミックなレッスンを提供。柔軟なカリキュラムや独自の教材を開発し、生徒の成長を支援する足場のあるアクティビティや実践的な英語使用を重視した授業設計を行う。教室での参加や自律学習を促進するためのウェブアプリやデジタルゲームを制作。語彙、アルファベット、スペリング、スピーキングなどのスキルを強化するためのICT教材を開発。地域の学校をつなぐコラボレーション型プロジェクトもコーディネート。",
            dates: "2021 - 現在",
            softSkills: ["カリキュラム開発", "学習評価", "ICTリテラシー育成", "ゲーム制作"],
            technicalSkills: ["HTML & CSS", "Javascript", "React", "Google Cloud Services"]
        },
        {
            id: 2,
            title: "オンライン英語講師",
            company: "Metaverse",
            link: "https://metaverse.skytalk.co.jp/",
            desc: "生徒が好きなゲームの世界観を活かして、オンライン英語レッスンを実施。ゲームプレイを通じて英語でのコミュニケーションを行い、協力してゲーム内の課題を達成。個々の学習ペースと興味に合わせたカリキュラムや教材を作成。学習の進捗やICTスキルの向上も記録・評価。",
            dates: "2025 - 現在",
            softSkills: ["リモート指導", "カリキュラム開発", "ICTリテラシー育成"],
            technicalSkills: []
        },
        {
            id: 3,
            title: "イベントコーディネーター",
            company: "Synergy Events Group",
            link: "https://www.synergyeventsgroup.com.au/",
            desc: "イベントの設営、撤去、技術要素の維持管理を担当し、スムーズな運営を支援。スケジューリング、会場レイアウト、技術要件などのイベント運営を効率的かつ柔軟に調整。会場運営者、クライアント、現場スタッフとの連携を通して、各イベントの要望に合ったスタイリングとAVソリューションを提供。クライアントの要望に応じた会場演出、フラワーアレンジメント、照明・音響機器の設置を担当。倉庫内の在庫管理や備品の記録・点検も行い、最適な在庫体制を維持。複数のイベントに対応するため、効率的な倉庫レイアウトを設計し、業務改善に貢献。",
            dates: "2015 - 2021",
            softSkills: ["イベント運営", "ロジスティクス調整", "在庫管理", "会場演出", "チームマネジメント", "課題解決"],
            technicalSkills: []
        },
        {
            id: 4,
            title: "ITサポート技術者",
            company: "NTC",
            link: "https://www.ntc.gov.au/",
            desc: "ITサポート: EDRMSのデータベース管理と記録管理、ユーザーの問題解決、ソフト・ハードウェアのトラブルシューティング、システム管理およびユーザープロファイルの管理。新CMSへのコンテンツ移行と構造設計、サイト利用やワークフローに関する職員研修を実施。20年以上のレガシーデータを新EDRMSおよび新公開サイトへ移行するプロジェクトの一員として活動。また、総務および受付業務も兼任。",
            dates: "2013 - 2015",
            softSkills: ["システム管理", "データ移行", "技術サポート", "職員研修", "トラブルシューティング"],
            technicalSkills: ["データベース管理", "SQL", "ネットワーキング"]
        }
    ]
}

export default employment